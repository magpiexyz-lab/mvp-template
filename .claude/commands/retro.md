...
3. **Spec files**
   ...
   - Read `.claude/failure-patterns.md` — list build issues encountered (if the file exists)
...
4. **Build Issues** — from `.claude/failure-patterns.md` (grouped by category; "None recorded" if file is empty or missing)
5. **Team Assessment** — answers to Q2-Q4
6. **Template Improvement Suggestions** — specific, actionable changes mapped to template components (e.g., "Add X to the bootstrap skill", "Change Y in CLAUDE.md Rule Z")
