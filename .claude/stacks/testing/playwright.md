---
assumes: [database/supabase, auth/supabase]
packages:
  runtime: []
  dev: ["@playwright/test"]
files:
...
> Assumes: `database/supabase` and `auth/supabase` (test user lifecycle uses Supabase Admin API)
...
- `blockAnalytics()` intercepts PostHog network requests via route interception — no app code changes needed
